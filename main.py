...
# Configuring flask app with gravatar
...
# defining user loader callback
@login_manager.user_loader
def load_user(user_id):
    return db.get_or_404(User, user_id)

# Creating adimin only decorator
def admin_only(func):
    @wraps(func)
...
def register():
    ...
    if register_form.validate_on_submit():
        ...
        if user:
            flash("You already Signed up with Email, Login Instead!","error")
            return redirect(url_for('login'))
        password = register_form.password.data
        hashed_salted_password = generate_password_hash(password,"pbkdf2:sha256",8)
